...
const project = {
  configFile: path.join(root, "nectar.config.ts"),
  appDir: path.join(root, "app"),
  outDir: path.join(root, ".nectar"),
};
const kind = (rel: string) => classifyPath(path.join(root, ...rel.split("/")), project);

describe("classifyPath", () => {
  test("config file", () => {
    expect(kind("nectar.config.ts")).toBe("config");
  });
  ...
  test("output, node_modules, git, tests, and non-source files are ignored", () => {
    expect(kind(".nectar/manifest.json")).toBe("ignored");
